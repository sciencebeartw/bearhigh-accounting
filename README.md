...
後續接手與產品設計請先讀 `docs/daily_operations_workflow.md` 與 `docs/numbers_workflow.md`。前者固定每月薪資、新生入班、舊生退班與跨視窗交接 SOP；後者整理 Numbers 的學生修課、學費合報/退費、老師課程名單與月薪資公式。實際日常點選與每月結算步驟請直接看 `docs/app_user_guide.md`。之後 UI 與資料模型均以這些規格為準。
...
- 同科換班：用生效日建立原班轉出與新班轉入，暫時換班可設換回日；換班不改學費、合報與已存薪資快照。
- 老師與課程主檔：分開實際授課老師與薪資歸屬老師，並可儲存人數分潤、保底超額、鐘點制與固定每堂等結構化計薪規則。
...
- 日常主操作入口：主導覽只保留「工作台」「學生帳務」「課程與名單」「老師薪資」四個入口；進退班、收退費、堂次計算、主檔、Numbers 移轉與稽核功能集中到工作台捷徑或「工具與設定」，避免新舊頁籤同時攤開。
...
- 新生／加科入班：日常進退班頁可一次建立學生、選課、應收、收款與入班生效日；學生會自動進入老師課程名單，且入班當天起列入計薪人數。同一人多科可依各科實際分攤金額逐科加入。
- 合報退班重算：一般新制支援 1 / 2 / 3 / 4 科 `21600 / 42200 / 61800 / 81400`，舊制支援 `16800 / 32600 / 47400 / 62200`。退班時先重算剩餘科目學費，再加上退掉科目已上堂數 × 單堂退費價（預設 1000）。學測／分科特殊套餐若未對到已確認規則，必須人工填寫剩餘學費，不自動猜。
- 逐堂人數與薪資條異動：堂次日期儲存後，畫面會逐堂列出計薪人數、當堂加入與當堂退出。加入日當堂起計，退出日當堂不計；單一老師薪資條下方會自動列本月進退班姓名。
- 行事曆特殊堂次：上課日期可用 `2026-06-19 | 不收費` 格式保留行事曆備註；系統會在月結標示「請覆核」，不會自行假設這堂是否付老師薪資。
- 底層紀錄：仍保留收款/退費流水與稽核資料，但操作畫面不再主打會計科目表、正式報表或 ERP 式功能。
- 匯出：本機草稿 JSON、學生大表/班級名單/年級總表/收款狀態與薪資相關 CSV。
- 工作台／本月結算入口：首頁先提供查學生帳務、記進退班、記收退費、補堂次四個日常動作，再接月底作業清單、薪資重算、列印與快照。從 Numbers 薪資表建立月結收進移轉區，只用來承接已完成舊月份；新月份直接在 APP 補堂次並結算。此入口沿用既有薪資結算與堂次日期邏輯，不另建第二套公式。
...
若要把一份已確認為唯一真相的 Numbers 快照完整切到 APP，必須先備份目前
`/accounting`，再使用完整主檔轉換器。它會建立學生、課程、選課、應收、
繳費流水、退費／抵扣備註與歷史老師月薪快照；舊匯入資料只封存或作廢，
不硬刪，輸出是一份相對於 `/accounting` 的 scoped multi-location update：

```bash
python3 src/import/build_master_import.py \
  --input /private/tmp/numbers_import.json \
  --existing /private/tmp/bearhigh_accounting_backup.json \
  --output /private/tmp/bearhigh_numbers_master_update.json \
  --summary-output /private/tmp/bearhigh_numbers_master_update_summary.json \
  --batch-id numbers-YYYYMMDD-current

firebase database:update /accounting \
  /private/tmp/bearhigh_numbers_master_update.json \
  --project bearhigh --force --disable-triggers
```

轉換器不會替 Numbers 沒有記載的付款方式、退費日期或退費所屬課程猜值；
自然合併收費也只建立一筆應收，避免物理、化學重複計價。
